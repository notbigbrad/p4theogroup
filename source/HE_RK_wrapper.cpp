...
namespace project
{
    //// Wrapper to use the generic runge kutta scheme in the fourth order to solve HE
    ...
    constexpr long double hydrostatic_equilibrium(long double r, long double P, long double M, long double rho)
    {
        return -(__gravitation * M * rho)/(r * r);
    }
    ...
    std::function<long double(long double, std::array<long double, 2>)> generic_first_order_ODE_2(function<long double(long double)> equation_of_state)
    {
        return [equation_of_state](long double r, std::array<long double, 2> y) -> long double
        {
            return hydrostatic_equilibrium(r, y[1], y[0], equation_of_state(y[1]));
        };
    }
    ...
    bool HE_criterion(long double r, array<long double, 2> y)
    {
    ...
    // Use generic solver to solve HE
    pair<vector<long double>, array<vector<long double>, 2>> HE_RK_wrapper(long double h, long double P_c, function<long double(long double)> equation_of_state)
    {
        ...
        // Solved criterion
        function<bool(long double r, array<long double, 2> y)> solved_criterion = HE_criterion;
	...
	ofstream file(f_name);
        ...
        for (long double P_c : P_c_list)
        {
            ...
            tie(r, y) = HE_RK_wrapper(h, P_c, equation_of_state);
