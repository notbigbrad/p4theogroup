...
namespace project
{
    ...
    pair< vector<long double>, array<vector<long double>, 2> >  LE_RK_wrapper(int n, function<bool( long double r, array<long double, 2> y )> solved_condition)
    {
    ...
    void LE_n(long double h, long double n_min, long double n_max, long double n_h, long double xi_max)
    {
        ...
        tie(r, y) = LE_RK_wrapper(n_min, LE_criterion);
        ...
        for (long double i = n_min + n_h; i <= n_max; i += n_h)
        {
            ...
            // Unwrap answer to ODE
            tie( r, y ) = LE_RK_wrapper(i, LE_criterion);
    ...
    {
        if (y[1] <= 0 || isnan(y[1]) || xi > 20.0L) { return true; }
        else { return false; }
    }
    ...
    void LE_mass_radius(long double n, long double K, long double P_c_min, long double P_c_max, int runs, function<long double(long double)> equation_of_state)
    {
        ...
        tie(_xi, _thetas) = LE_RK_wrapper(n, LE_criterion2);
